...
-- 4. Swap invites' primary key from code -> id.
...
-- 5. Drop the columns the search-based flow no longer needs.
alter table invites drop column code;
alter table invites drop column name;

-- 6. Add attendee email + case-insensitive lookup indexes to support search.
